...
def vis_segmentation_stream(image, seg_map, index, output_video):
    ...
    # img = np.fromstring(s, np.uint8).reshape((1920, 1080)
    

    # Show visualization in a streaming fashion.
    f = BytesIO()
    plt.savefig(f, format='jpeg')
    npImg = cv.imdecode(np.fromstring(f.getvalue(), dtype=np.uint8), 1)
    print(seg_image.shape)
    output_video.write(seg_image)
...
def run_visualization_video(frame, index, output_video):
    """Inferences DeepLab model on a video file and stream the visualization."""
    original_im = Image.fromarray(frame[..., ::-1])
    seg_map = MODEL.run(original_im)
    
    vis_segmentation_stream(original_im, seg_map, index, output_video)
...
output_video = cv.VideoWriter("deeplab_v3_s.avi", fourcc, 30, (1920, 1080))
...
try:
    for i in range(num_frames):
        _, frame = video.read()
        if not _: break
        run_visualization_video(frame, i, output_video)
